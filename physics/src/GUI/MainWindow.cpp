...
LRESULT MainWindow::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
        ...
        Utils::DPIScale::Initialize(m_hwnd);
        return 0;
...
void MainWindow::OnLButtonDown(int pixelX, int pixelY, DWORD flags) {
    const float dipX = Utils::DPIScale::PixelsToDipsX(pixelX);
    const float dipY = Utils::DPIScale::PixelsToDipsY(pixelY);
...
HRESULT MainWindow::InsertTriangle(float x, float y) {
    try
    {
        ...
        // eng.Selection().get()->set_color(D2D1::ColorF(colors[nextColor]));
        ptMouse = D2D1::Point2F(x, y);
        //((Triangle*)eng.Selection().get())->set_dims(2.0f, 2.0f);
        nextColor = (nextColor + 1) % ARRAYSIZE(colors);
    }
    catch (std::bad_alloc)
    {
